...
def save_partitions(save_to, layers, partitions):
    """"""
    for bidx in range(len(partitions)):
        ...
        for idx in batch:
            for param in layers[idx].parameters():
                ...
                # append to file
                with open(os.path.join(save_to, "{}.bin".format(bidx)), "a+b") as ofile:
    ...
    for d in domains:
        ...
        for m_name in model_names:
            ...
            for p_name in tqdm(perturbs):
                ...
                if d == "nlp":
                    model_class, _, _, config_class = models_dict[m_name]
                    model = load_nlp_model(
                        param_path, model_class, config_class)
                elif d == "vision":
                ...
                print("{} total-size {}, num-layers {} partitions:".format(p_name, sum(sizes), len(layers)), parts)
                save_to = os.path.join(partition_dir, d, m_name, p_name)
                if not os.path.exists(save_to):
                    os.makedirs(save_to)
                save_partitions(save_to, layers, parts)
